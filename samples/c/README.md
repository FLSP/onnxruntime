...
A minimal C program demonstrating basic ONNX Runtime inference using the C API. It loads an ONNX model that adds two float tensors (`C = A + B`), runs inference, and verifies the result.
...
```
tar -xf onnxruntime-linux-x64-1.26.0.tgz
```

This creates a directory like `onnxruntime-linux-x64-1.26.0/` containing `include/` and `lib/` subdirectories.
...
- Uses `onnxruntime_c_api.h` instead of `onnxruntime_cxx_api.h`
- Manual memory management: all ORT objects must be explicitly released
- Uses `OrtApi` function pointers instead of C++ wrapper classes
- Error handling via `OrtStatus*` return values with `CHECK_ORT_STATUS` macro
- Explicit allocation and deallocation of names and tensors
